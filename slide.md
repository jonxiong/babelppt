...
:::note
babel\`beibl\
大家好今天给大家分享一下我对babel一个初步的理解，有什么疑问的地方可以指出来一起探讨
:::
...
:::note
babel \`beibl\ 的相关历史：
我第一次打开搜索引擎 查询关于Babel的资料时，出现的竟然是关于 Babel 的传说还有电影
...
没错就一句话，Babel是一个现代JS编译器, 有很多**优秀的es6,es7新语法**（比如说 常见的箭头符号，class, promise，async,await 等等）都不能直接在**只支持低版本语法的浏览器**中运行, 各个浏览器对 JS版本的支持各不相同。为了解决这个“沟通不畅”的问题，所以就有了 Babel，Babel 的出现使得我们可以无须顾忌的去使用新的的语法

我们可以通过这个网站来看一下babel的转换

我们会发现 babel 可以认识代码并且更改代码

所以babel是如何转换es6/es7的新语法呢？
...
2. 转换（transform）-- 操作AST，把ES2015+的部分转换为ES5, 这也是我们可以通过配置bebel插件去操作的部分
3. 生成（generate）-- 将更改后的AST，再变回JS代码。

注意babel本身不具有任何转化功能，它把转化的功能都分解到 plugin 里面。因此当我们不配置任何插件时，经过babel的代码和输入是相同的
...
但是实际开发过程中，如果每一个插件都需要手动去添加，会影响效率，并且我们可能也不清楚 浏览器环境到底需要哪些转换插件，
为此 babel 为我们提供了presets**预设**就是plugins 的组合，你也可以理解为是套餐

官方提供了如下几个
@babel/preset-env 推荐用的最多
@babel/preset-flow
@babel/preset-react
@babel/preset-typescript
...
在我们开发遇到的比如：代码压缩、代码高亮，css预处理器、elint等其实都是基于AST来实现的，所以了解一下AST还是很有必要的

AST是什么，我们可以先通过这个网站来感受下
...
在解析过程中有两个阶段：词法分析和语法分析，词法分析把字符串形式的代码转换为令牌（tokens）流，令牌类似于AST中节点；而语法分析 则会把一个令牌流转换成AST的形式，同时这个阶段会把令牌中的信息转换成AST的表述结构。

Babel中的解析器是babylon/`baibilen/ 。我们来体验一下：
...
在这个阶段，Babel接受得到AST并对其进行深度优先遍历，在此过程中对节点进行添加、更新及移除操作。这部分也是Babel插件介入工作的部分
...
经过上面两个阶段，需要转译的代码已经经过转换，生成了新的AST，最后一个步就是根据这个AST 来输出代码. Babel中的babel-generator对应这一步操作

在code字段里，我们可以看到里面新生成的代码

上面讲的这几个库，还有更多细节，有兴趣的同学可以再细入研究

最后 这些库的集合，就是我们的Babel
